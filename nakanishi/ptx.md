[](ファイル名はコマンド名.md)
# ptx 
ptxコマンドは、与えられたテキストファイルを処理し、単語の順列索引を生成する。順列索引は、単語をアルファベット順にソートし、単語ごとにその出現箇所や参照情報を提供する。順列索引は、単語を検索したり、単語の出現パターンを分析したりするのに役立ちます。
...
- **-F** 
  ...
  行の切り詰めを示すためのフラグを指定するために使用さる。行の切り詰めとは、テキストが行の幅に収まらずに途中で切り捨てられた場合に、それを示すための特定のフラグや記号を挿入することを指しす。
